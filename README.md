...
| 后端（FastAPI） | **22001** | 健康检查 `http://127.0.0.1:22001/api/health` |

```bash
./start.sh
```

运行日志写入仓库根目录 **`.dejavu/`**（已加入 `.gitignore`）：后端（含 uvicorn 访问日志）在 **`backend.log`**，前端 dev 输出在 **`frontend.log`**。`start.sh` 使用 `tee`，**终端里也会打印一份**，并追加到上述文件。

Docker Compose 里同样用 **`tee -a`**：既写入挂载的 **`.dejavu/*.log`**，又送到容器 **stdout**，因此 **`docker logs <容器名>`** 能看到与文件一致的输出。
...
**逮虾户**是一套调度层：把分布在**局域网**或**公网远端**上的 OpenClaw **节点（下文戏称「龙虾」）**接进来，统一分发任务、回收结果、做健康与权限治理。本仓库当前为规划与实现入口；集成 OpenClaw 时，核心是对齐其 **Gateway 协议**，而不是自造一套私有 RPC。
...
| 场景 | 说明 |
|------|------|
| 局域网 | 办公室/家里多台机器跑 OpenClaw Gateway + Node，逮虾户负责发现地址、鉴权、任务路由。 |
| 远端 | 云主机或分支办公室通过 TLS + Token 接入同一套 Gateway 或经逮虾户做聚合。 |

逮虾户本身不替代 OpenClaw 的执行能力；它更像是 **Operator 侧的编排与策略层**（或旁路控制面），与 Gateway 的 **WebSocket 控制面 + Node 能力声明**对齐。
...
逮虾户若要把「龙虾」当算力/工具池用，本质是：**以合规方式连上 Gateway，按 Node/Operator 能力做任务拆分与授权**，而不是绕过 Gateway 直接调模型。
...
涉及在 Node 上跑命令等场景时，Gateway 侧有 **`exec.approval.request` / `exec.approval.resolve`** 等流程；逮虾户若代理「批准」类操作，需要 **Operator 侧具备 `operator.approvals` 等 scope**，并理解广播事件语义。
...
1. **固定 Gateway 拓扑**  
   明确：逮虾户是只连**一个** Gateway，还是多 Gateway 聚合；端口（默认常见为 **18789**，以实际部署为准）、是否 HTTPS/WSS。
...
4. **区分「调度逻辑」与「Gateway 能力」**  
   在逮虾户内实现：任务队列、节点选择（按 `caps`/`commands`）、超时与重试；**不要**重复实现 OpenClaw 已提供的会话与执行语义，尽量通过 Gateway 暴露的 API 调用。
...
- `docs/`：逮虾户自己的消息格式（若在与 Gateway 之外还有一层总线）。  
...
**逮虾户**：调度与编排的一方；**龙虾**：接入的 OpenClaw 节点。命名仅为工程内沟通用语，与 OpenClaw 官方术语对照关系见上文 **operator / node**。
